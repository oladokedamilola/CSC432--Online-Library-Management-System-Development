...
from .recommendation_utils import get_book_recommendations 
...
def book_list(request):
    ...
    # Generate recommendations for the current page of books
    recommendations = {}
    for book in books:
        recommendations[book.id] = get_book_recommendations(book.id, top_n=3)
    ...
    context = {
        ...
        'genres': genres,
        'recommendations': recommendations,  # Include recommendations
    }
...
def book_detail(request, pk):
    ...
    # Find similar books in the same category, excluding the current book
    recommendations = Book.objects.filter(category=book.category).exclude(id=book.id)[:5]

    return render(
        request, 
        'books/book_detail.html', 
        {'book': book, 'userbook': userbook, 'recommendations': recommendations}
    )
